...
static std::string createRockchipHDMIStream(const CameraSettings& settings) {
  std::stringstream ss;
  ss << createRockchipV4L2Pipeline(0, settings);
...
static std::string createImageSavingCodec(
  ...
  std::stringstream ss;
  // don't forget the white space before the " t." !
  ss << " t. ! queue ! decodebin ! videorate ! videoscale ! videoconvert ! ";
  ss << "video/x-raw,width=960,height=540,format=GRAY8,framerate=1/";
  ss << seconds;
